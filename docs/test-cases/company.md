...
1. Verify that the Create Company button opens the Create Company form/modal successfully.
2. Verify that user is able to open the Create a New Company screen successfully.
3. Verify that Company Name field accepts valid company name and is marked as mandatory.
4. Verify that Market Vertical dropdown opens correctly and displays all available options.
5. Verify that user is able to search and select a value from Market Vertical dropdown.
6. Verify that Strategic Partnership Status dropdown opens and displays all valid options.
7. Verify that user can select any Strategic Partnership Status successfully.
8. Verify that Company Domain field accepts a valid domain format.
9. Verify that No. of Employees field accepts numeric values only.
10. Verify that Revenue field accepts valid numeric input.
11. Verify that Address field allows user to search and select an address from Google Maps suggestions.
12. Verify that selected address is correctly reflected on the map.
13. Verify that Create Company button gets enabled after filling all mandatory fields.
14. Verify that company is created successfully when user clicks on Create Company with valid data.
15. Verify that newly created company is visible in the company listing after successful creation.
16. Verify that Cancel button closes the Create Company screen without saving data.
17. Verify that error message is displayed when user tries to create a company without entering Company Name.
18. Verify that error message is shown when Market Vertical is not selected and user clicks Create Company.
19. Verify that Create Company button remains disabled if mandatory fields are missing.
20. Verify that Company Domain field does not accept invalid domain formats.
21. Verify that No. of Employees field does not accept alphabetic or special characters.
22. Verify that Revenue field does not accept invalid characters or negative values.
23. Verify that user cannot submit the form without selecting an Address.
24. Verify that invalid or random text entered in Address field does not allow company creation.
25. Verify that dropdown values do not disappear or overlap when opened multiple times.
26. Verify that selected Market Vertical value is not reset unexpectedly on form interaction.
27. Verify that selected Strategic Partnership Status remains intact after interacting with other fields.
28. Verify that user is prevented from creating duplicate company with same name.
29. Verify that form data is not saved when user clicks Cancel.
30. Verify that Create Company button is not accessible for users without permission (proper access control for SM and other roles).

## Describe: Company Listing & Grid UI
31. Verify that the Companies listing page loads successfully and displays charts, filters, and the companies grid.
32. Verify that the total Companies count is displayed and matches the grid pagination total.
33. Verify that the grid displays N/A for empty fields consistently without UI break.
34. Verify that horizontal scrolling allows viewing all columns without layout breaking.
35. Verify that the grid retains current filters/sort after page refresh (if expected behavior).
36. Verify that the Companies listing page shows a friendly error state if the data unable to load.
37. Verify that charts do not overlap or break layout when data is missing or zero.
38. Verify that sorting does not break when the column contains N/A values (N/A handled consistently).
39. Verify that sorting remains stable and does not randomize results when toggled quickly.
40. Verify that the grid does not become unclickable after closing the filters panel (no overlay remains).
41. Verify that proper validation messages are displayed instead of generic errors.
42. Verify that system does not crash or freeze when user rapidly opens and closes dropdowns.

## Describe: Analytics & Charts
43. Verify that the 'Companies by Contracts' donut chart renders with Active vs Inactive contract breakdown.
44. Verify that the 'Companies by Market Verticals' donut chart renders with market vertical distribution and legend.
45. Verify that the Companies trend chart renders and is aligned with the selected time range on the x‑axis.

## Describe: Search Functionality
46. Verify that the Search by Company input allows searching by full company name and returns matching results.
47. Verify that the Search by Company input supports partial text search and updates the grid accordingly.
48. Verify that clearing the search input restores the default company list results.
49. Verify that search with special characters (e.g., @, #, %) does not crash and returns valid 'no results' behavior.
50. Verify that search with very long text does not break UI and is handled gracefully.
51. Verify that the grid shows 'No results' state when filters return no matching companies.

## Describe: Filter Management (Market Vertical & More Filters)
52. Verify that the Market Vertical filter dropdown opens and shows all available market vertical options.
53. Verify that the Market Vertical filter supports searching within the dropdown list (Search by Industry).
54. Verify that the user can select one market vertical option and the grid updates accordingly.
55. Verify that the user can select multiple market vertical options (checkbox multi-select) and the grid updates accordingly.
56. Verify that deselecting a selected market vertical option removes the filter and updates the grid.
57. Verify that Market Vertical dropdown does not close unexpectedly while selecting multiple options.
