...
- **Additional Requirement Validation:**
  - Check that `{{REQUIREMENT_DESCRIPTION}}` appears at least 4 times:
    1. In file header comment block
    2. In `test.describe()` label
    3. In `console.log('[REQUIREMENT]')` at start
    4. In comment above tests
  - Warn if requirement description is missing or incomplete
- Report format:
  ```
  [SYNTAX] ✅ No syntax errors found
  [REQUIREMENT] ✅ Exact requirement found 4 times in file
  — OR —
  [SYNTAX] ❌ 2 errors found:
    Line 45: Missing import for 'ContractModule'
    Line 102: Undefined variable 'serviceTotal'
  [REQUIREMENT] ⚠️ Requirement description only found 1 time (expected 4)
  ```
...
### Phase 6: Run Tests with Browser Validation (Single Session, No Per-Test Open/Close)

Execute all tests in a **single browser session** — login once, run all tests, close once:
...
2. **Single Session Strategy (NO per-test browser open/close):**
   ```javascript
   // ✅ CORRECT: Open browser ONCE
   let sharedBrowser, sharedContext, sharedPage;
   ...
   test.beforeAll(async ({ browser }) => {
     ...
     // Login ONCE at start
     const baseUrl = process.env.BASE_URL;
     await sharedPage.goto(baseUrl, { waitUntil: 'networkidle' });
     await performLogin(sharedPage);  // Login happens ONCE
     console.log('[SESSION START] Single browser session opened + logged in');
   });
   
   test.beforeEach(async () => {
     // Optional: Reset application state between tests (if needed)
     // - Navigate to home page
     // - Clear form state
     // But do NOT close/reopen browser!
     await sharedPage.goto(`${baseUrl}/app/home`);
   });
   
   test.afterAll(async () => {
     // Close browser ONCE at end
     await sharedContext.close();
     console.log('[SESSION COMPLETE] Browser closed');
   });
   ```

3. **Execute Tests (Single Session):**
   ```bash
   # All tests run in ONE browser session (sequential execution)
   HEADLESS=false ENV_NAME=uat npx playwright test {{TEST_OUTPUT_FILE}} --project=chromium --workers=1
   ```
   
   **Alternative (Parallel within same session):**
   ```bash
   # Tests run in parallel but share same browser context (faster)
   HEADLESS=false ENV_NAME=uat npx playwright test {{TEST_OUTPUT_FILE}} --project=chromium --workers=4
   ```

4. **Single Session Benefits:**
   - ✅ **40–60% faster execution** — No browser open/close overhead per test
   - ✅ **Login once** — All tests use same authenticated session
   - ✅ **Shared state** — Tests can use data from previous tests if needed
   - ✅ **Single browser tab** — Visual tracking of all test steps in one window
   - ✅ **Lower resource usage** — One browser instance vs. N test instances
   - ✅ **Cleaner logs** — Single login sequence instead of N repeated logins

5. **Logout/Reset Strategy (Optional per test):**
   ```javascript
   test.beforeEach(async () => {
     // Option A: Do nothing — tests share session (FASTEST)
     // This works if tests don't interfere with each other
     
     // Option B: Navigate to home page to reset UI state
     await sharedPage.goto(`${baseUrl}/app/home`);
     
     // Option C: Clear cookies to partially reset session
     await sharedContext.clearCookies();
     
     // Option D: Full logout + re-login (SLOWEST but most isolated)
     // await performLogout(sharedPage);
     // await performLogin(sharedPage);
   });
   ```
   
   **Recommendation:** Use Option B (navigate to home) for balance of speed + isolation.

6. **Capture Results (Single Session):**
   - Per-test pass/fail status
   - Error messages and stack traces
   - Screenshots on failure (auto-saved to `screenshots/`)
   - Test execution time per case
   - Total suite execution time (includes ONE login, not per-test)

7. **Report Format:**
   ```
   [SESSION START] Opening single browser session...
   [LOGIN] Logging in as {{USER}}...
   [SESSION READY] 1 browser, 1 context, 1 page — ready for all tests
   ...
   [PASS] TC-CONTRACT-DELETE-005 ✅ (1.9s)
   [PASS] TC-CONTRACT-DELETE-006 ✅ (2.1s)
   [PASS] TC-CONTRACT-DELETE-007 ✅ (1.7s)
   [PASS] TC-CONTRACT-DELETE-008 ✅ (1.5s)
   [PASS] TC-CONTRACT-DELETE-009 ✅ (2.4s)
   [PASS] TC-CONTRACT-DELETE-010 ✅ (1.8s)
   
   [RUN COMPLETE] 9 passed, 1 failed | Total: 18.3s
     (vs ~80s with per-test open/close overhead)
   [SESSION COMPLETE] Browser closed
   ```
...
## Step 3 — Test Structure (Single Session Pattern)

Organize each spec file **with single browser session** (open once, close once):
 ...
 * ═══════════════════════════════════════════════════════════════════════════
 * 
 * This test suite validates the exact requirement stated above.
 * All tests run in a single browser session to maximize performance.
 */

test.describe('{{MODULE_NAME}} — {{REQUIREMENT_DESCRIPTION}}', () => {
  // ── Shared session (reused across ALL tests) ──
  let sharedBrowser, sharedContext, sharedPage;
  let contractModule;
  const baseUrl = process.env.BASE_URL || 'https://uat.sales.teamsignal.com';

  // ── Open browser ONCE before all tests ──
  test.beforeAll(async ({ browser }) => {
    console.log('[SESSION START] Opening single browser session...');
    console.log(`[REQUIREMENT] {{REQUIREMENT_DESCRIPTION}}`);
    
    sharedBrowser = browser;
    sharedContext = await browser.newContext();
    sharedPage = await sharedContext.newPage();
    contractModule = new ContractModule(sharedPage);

    // Login ONCE at start
    console.log('[LOGIN] Navigating to ' + baseUrl);
    await sharedPage.goto(baseUrl, { waitUntil: 'networkidle' });
    
    // TODO: implement login once
    // await contractModule.loginAsUser(email, password);
    console.log('[SESSION READY] Browser session established and logged in');
  });

  // ── Reset state between tests (optional) ──
  test.beforeEach(async () => {
    console.log('[TEST SETUP] Resetting application state...');
    // Navigate to home or clear form state between tests
    // But do NOT close/reopen browser!
    await sharedPage.goto(`${baseUrl}/app/home`, { waitUntil: 'domcontentloaded' });
    await sharedPage.waitForTimeout(300);  // Brief pause for page load
  });
  ...
  test.afterAll(async () => {
    console.log('[SESSION CLEANUP] Closing browser session...');
    await sharedContext.close();
    console.log('[SESSION COMPLETE] Browser closed');
  });

  // ─── Individual Tests ────────────────────────────────────────────────────
  // All tests below validate: "{{REQUIREMENT_DESCRIPTION}}"
  
  test('TC-EXAMPLE-001 | Exact scenario name from documentation', async () => {
    console.log('[TEST START] Running: TC-EXAMPLE-001');
    
    console.log('[STEP 1] Perform first action...');
    await contractModule.someAction();
    
    console.log('[ASSERT] Verify expected behavior...');
    await expect(contractModule.someElement).toBeVisible();
    
    console.log('[DONE] Test completed successfully');
  });

  test('TC-EXAMPLE-002 | Another test scenario name from documentation', async () => {
    console.log('[TEST START] Running: TC-EXAMPLE-002');
    // This test uses the same sharedPage + contractModule
    // from the beforeAll session setup
    console.log('[DONE] Test completed successfully');
  });
...
1. **Requirement Comment Block** (top of file):
   ```javascript
   /**
    * REQUIREMENT: {{REQUIREMENT_DESCRIPTION}}
    * 
...
2. **test.describe() Label**:
   ```javascript
   test.describe('{{MODULE_NAME}} — {{REQUIREMENT_DESCRIPTION}}', () => {
   ```
   Uses the EXACT requirement description in the suite name.

3. **Console Log at Start**:
   ```javascript
   console.log(`[REQUIREMENT] {{REQUIREMENT_DESCRIPTION}}`);
   ```
   Prints requirement at the start of test run.

4. **Comment Above Tests**:
   ```javascript
   // ─── Individual Tests ───────────────────────────────────────────────────
   // All tests below validate: "{{REQUIREMENT_DESCRIPTION}}"
   ```
   Reminds developers what they're validating.

5. **Individual Test Titles**:
   - Use exact names from documentation
   - Example: `test('TC-CONTRACT-DELETE-001 | Delete a single service updates the grand total', async () => {`
   - Each test title mirrors the doc scenario exactly

**Key Differences from Per-Test Pattern:**
- ✅ `test.beforeAll()` — Opens browser **once** for entire suite
- ✅ `test.beforeEach()` — Resets state only (no browser open/close)
- ✅ `test.afterAll()` — Closes browser **once** after all tests
- ✅ `sharedPage` — Single page object reused across all tests
- ✅ No `({ page })` parameter in individual tests (they use `sharedPage`)
- ✅ **EXACT requirement description appears 4 times** (comment block, describe label, console log, test comment)

---

## Step 3b — Auto-Generate Single Session Pattern

When generating tests, Claude Agent should **automatically use the single session pattern**:

1. **Include Requirement Description (4 Places):**
   
   **Place 1: File Header Comment Block**
   ```javascript
   /**
    * ═══════════════════════════════════════════════════════════════════════════
    * REQUIREMENT: {{REQUIREMENT_DESCRIPTION}}
   ...
   **Place 2: test.describe() Label**
   ```javascript
   test.describe('{{MODULE_NAME}} — {{REQUIREMENT_DESCRIPTION}}', () => {
   ```
   
   **Place 3: Console Log at Session Start**
   ```javascript
   console.log(`[REQUIREMENT] {{REQUIREMENT_DESCRIPTION}}`);
   ```
   
   **Place 4: Comment Above Tests**
   ```javascript
   // All tests below validate: "{{REQUIREMENT_DESCRIPTION}}"
   ```

2. **Detect if output file is new:**
   - If new file → generate with full header + `beforeAll` + `afterAll` pattern
   - If existing file → append tests, keep existing session setup

3. **Auto-inject Session Setup:**
   ```javascript
   test.beforeAll(async ({ browser }) => {
     // Session setup (once)
     console.log(`[REQUIREMENT] {{REQUIREMENT_DESCRIPTION}}`);
...
4. **Reuse Shared Variables:**
   - All individual tests reference `sharedPage`, `contractModule`
   - No parameters like `({ page })` in test function signatures
   - No creating new contexts per test

5. **Test Title Format:**
   - Use exact scenario name from documentation
   - Format: `test('TC-XXX-001 | Exact scenario name from docs', async () => {`
   - Example: `test('TC-CONTRACT-DELETE-001 | Delete a single service updates the grand total', async () => {`

6. **Validate Single Session Pattern:**
   - ✅ Check requirement description appears exactly 4 times
   - ✅ Check that no test calls `browser.newContext()`
   - ✅ Check that no test has duplicate login code
   - ✅ Check that `beforeAll` exists and sets up shared variables
   - ✅ Check test titles match documentation scenario names exactly
   - ⚠️ Warn if pattern is not followed
...
## Performance Comparison: Single Session vs Per-Test

### Execution Time Breakdown (10 tests example)

**❌ Per-Test Pattern (OLD — inefficient):**
```
Test 1:  Browser open (2s) + Login (3s) + Test (2.3s) + Close (1s) = 8.3s
Test 2:  Browser open (2s) + Login (3s) + Test (1.8s) + Close (1s) = 7.8s
Test 3:  Browser open (2s) + Login (3s) + Test (2.1s) + Close (1s) = 8.1s
Test 4:  Browser open (2s) + Login (3s) + Test (1.9s) + Close (1s) = 7.9s
Test 5:  Browser open (2s) + Login (3s) + Test (2.0s) + Close (1s) = 8.0s
Test 6:  Browser open (2s) + Login (3s) + Test (3.2s) + Close (1s) = 9.2s
Test 7:  Browser open (2s) + Login (3s) + Test (1.7s) + Close (1s) = 7.7s
Test 8:  Browser open (2s) + Login (3s) + Test (1.5s) + Close (1s) = 7.5s
Test 9:  Browser open (2s) + Login (3s) + Test (2.4s) + Close (1s) = 8.4s
Test 10: Browser open (2s) + Login (3s) + Test (1.8s) + Close (1s) = 7.8s
────────────────────────────────────────────────────────────────────
TOTAL: 80.7 seconds (50 seconds overhead = 62% wasted on open/close/login!)
```

**✅ Single Session Pattern (NEW — optimized):**
```
Setup:   Browser open (2s) + Login (3s) = 5s

Test 1:  Navigate home (0.3s) + Test (2.3s) = 2.6s
Test 2:  Navigate home (0.3s) + Test (1.8s) = 2.1s
Test 3:  Navigate home (0.3s) + Test (2.1s) = 2.4s
Test 4:  Navigate home (0.3s) + Test (1.9s) = 2.2s
Test 5:  Navigate home (0.3s) + Test (2.0s) = 2.3s
Test 6:  Navigate home (0.3s) + Test (3.2s) = 3.5s
Test 7:  Navigate home (0.3s) + Test (1.7s) = 2.0s
Test 8:  Navigate home (0.3s) + Test (1.5s) = 1.8s
Test 9:  Navigate home (0.3s) + Test (2.4s) = 2.7s
Test 10: Navigate home (0.3s) + Test (1.8s) = 2.1s

Cleanup: Browser close (0.5s) = 0.5s
────────────────────────────────────────────────────────────────────
TOTAL: 27.2 seconds (4× FASTER!)
```

### Results Summary

| Metric | Per-Test | Single Session | Improvement |
|---|---|---|---|
| **Total Time** | 80.7s | 27.2s | **66% faster** |
| **Overhead** | 50s (62%) | 5.5s (20%) | **90% less overhead** |
| **Browser Opens** | 10 | 1 | **10× fewer** |
| **Login Operations** | 10 | 1 | **10× fewer** |
| **Resource Usage** | Very high | Low | **Much lighter** |
...
| **SINGLE SESSION** | Open browser ONCE; run all tests; close ONCE | NO per-test browser open/close |
| **LOGIN ONCE** | Login before all tests, not per-test | Reuse authenticated session |
| **NO CONTEXT RECREATION** | Use shared context across all tests | Avoid `browser.newContext()` per-test |
...
[REQUIREMENT] ✅ Exact requirement "{{REQUIREMENT_DESCRIPTION}}" found 4 times
...
═══════════════════════════════════════════════════════════════
[SYNTAX] ✅ No syntax errors (ESLint passed)
[REQUIREMENT] ✅ Exact requirement found 4 times in generated file:
  1. Line 5:   /** REQUIREMENT: Verify deleting a service updates totals... */
  2. Line 15:  test.describe('Contract Module — Verify deleting a service...')
  3. Line 37:  console.log('[REQUIREMENT] Verify deleting a service updates...')
  4. Line 81:  // All tests below validate: "Verify deleting a service..."
[DUPLICATION] ⚠️ 1 pattern (suggestion provided)
[QUALITY] ✅ All tests logged, timed, safe

# PHASE 6 — EXECUTION (SINGLE SESSION)
═══════════════════════════════════════════════════════════════
[SESSION START] Opening single browser session...
[LOGIN] Logging in as user@example.com...
[SESSION READY] 1 browser instance, 1 context, 1 page ready

[PASS] TC-CONTRACT-DELETE-001 ✅ (2.3s)
[PASS] TC-CONTRACT-DELETE-002 ✅ (1.8s)
[PASS] TC-CONTRACT-DELETE-003 ✅ (2.1s)
[FAIL] TC-CONTRACT-DELETE-004 ❌ (timeout at selector)
...
[RUN COMPLETE] 9 passed, 1 failed | Total: 27.2s (vs 80.7s per-test)
[SESSION CLEANUP] Closing browser...
[SESSION COMPLETE] Browser closed

# PHASE 7 — AUTO-FIX
═══════════════════════════════════════════════════════════════
[FIX ATTEMPT 1/3] Selector alternatives
[CHANGE] Changed selector from [data-testid="spinbutton"] to getByRole('spinbutton')
[RETRY] TC-CONTRACT-DELETE-004...
[PASS] ✅ Test fixed! (now 2.5s)

═══════════════════════════════════════════════════════════════
✅ SUCCESS — ALL 10 TESTS PASSING
📊 Execution: 27.2s (66% faster due to single session)
═══════════════════════════════════════════════════════════════
```
