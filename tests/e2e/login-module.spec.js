...
test.describe("Login Module — Signal App", () => {
  ...
  test("TC-002 | Valid credentials navigate to dashboard", async ({ page }) => {
    ...
    await expect(
      page.getByText("Sales Insights", { exact: true }).first(),
    ).toBeVisible();
  });

  // TC-003 | Empty Form Submit
  test("TC-003 | Empty form submission shows required fields error", async ({
    page,
  }) => {
  ...
  test("TC-004 | Email only with empty password shows server error", async ({
    page,
  }) => {
  ...
  test("TC-005 | Wrong password shows invalid credentials error", async ({
    page,
  }) => {
  ...
  test("TC-010 | Pressing Enter on password field submits the form", async ({
    page,
  }) => {
    await loginPage.fillEmail(VALID_EMAIL);
    await loginPage.fillPassword(VALID_PASS);
  ...
  test("TC-011 | Forgot Password link navigates to forgot-password page", async ({
    page,
  }) => {
  ...
  test("TC-012 | Error clears when correct credentials entered after failed attempt", async ({
    page,
  }) => {
    await loginPage.login(VALID_EMAIL, WRONG_PASS);
  ...
  test("TC-014 | User can log out and is redirected to the login page", async ({
    page,
  }) => {
    test.setTimeout(180_000);
    await performLogin(page);
    await page.goto(`${env.baseUrl}/app/sales/dashboard`, {
